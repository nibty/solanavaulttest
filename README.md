# OpenBao + Solana Transaction Signing Demo

Signs Solana transactions using OpenBao's transit secrets engine. The Ed25519 private key never leaves OpenBao.
...
```bash
BAO_TOKEN=<service-token> yarn transfer -- --to <RECIPIENT_ADDRESS> --amount 0.01
```

## How It Works

1. OpenBao's transit engine holds an Ed25519 key (`guardian-1`)
2. The CLI fetches the public key from OpenBao — derives the Solana address
3. Builds a SOL transfer transaction
4. Sends the serialized message to OpenBao for signing
5. OpenBao returns the Ed25519 signature (private key never leaves OpenBao)
...
- `src/vault.ts` — OpenBao HTTP API client (public key fetch + signing)
...
- Inject `BAO_TOKEN` via systemd environment variable, not `.env` files
